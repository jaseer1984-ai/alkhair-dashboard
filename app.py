...
# - Sidebar always visible (Branch filter pinned there)
# - URL input hidden by default; can be revealed from sidebar (Advanced)
# - Branch Overview shows KPI cards per branch
# - Daily Trends uses a new model: metric tabs (Sales / NOB / ABV) with area/line
...
def apply_css():
    st.markdown(
        ...
        .pill {
            display:inline-block; padding:4px 10px; border-radius:999px; font-weight:700; font-size:.80rem;
        }
...
def load_workbook_from_upload(file) -> Dict[str, pd.DataFrame]:
    ...
    return {
        sn: xls.parse(sn).dropna(how="all").dropna(axis=1, how="all")
        for sn in xls.sheet_names
        if not xls.parse(sn).dropna(how="all").dropna(axis=1, how="all").empty
    }
...
def _metric_area(df: pd.DataFrame, y_col: str, title: str) -> go.Figure:
    """Area/line chart per metric with one trace per branch."""
    if df.empty or "Date" not in df.columns or df["Date"].isna().all(): return go.Figure()
    daily = (df.groupby(["Date","BranchName"]).agg({y_col: "sum" if y_col!='ABVActual' else "mean"}).reset_index())
    fig = go.Figure()
    ...
    for i, br in enumerate(daily["BranchName"].unique()):
        ...
        fig.add_trace(go.Scatter(
            ...
            fill="tozeroy",  # area
            hovertemplate=f"<b>{br}</b><br>Date: %{ { 'x' } }<br>Value: %{ { 'y' } :,.0f}<extra></extra>",
        ))
...
def render_branch_cards(bp: pd.DataFrame):
    ...
    # small grid: three cards per row
    cols_per_row = 3
    items = list(bp.index)
    for i in range(0, len(items), cols_per_row):
        ...
        for j, br in enumerate(items[i:i+cols_per_row]):
            with row[j]:
                r = bp.loc[br]
                avg_pct = np.nanmean([r.get("SalesPercent",0), r.get("NOBPercent",0), r.get("ABVPercent",0)])
                cls = branch_status_class(float(avg_pct if pd.notna(avg_pct) else 0))
                st.markdown(
                    f"""
                    <div class="card">
...
def main():
    ...
    with st.sidebar:
        ...
        if show_url:
            url = st.text_input("Published Google Sheets URL", value=config.DEFAULT_PUBLISHED_URL,
                                help="Paste a Google Sheets 'Publish to web' URL.")
    ...
    # ----- Branch filter pinned in sidebar
    if "BranchName" in df.columns:
        with st.sidebar:
    ...
    # ----- Date filter in main (optional to move later)
    if "Date" in df.columns and df["Date"].notna().any():
    ...
    with t1:
        ...
        if "branch_performance" in k and not k["branch_performance"].empty:
            ...
            st.dataframe(
                ...
                .rename(columns={"SalesPercent":"Sales %","NOBPercent":"NOB %","ABVPercent":"ABV %","SalesActual":"Sales (Actual)","SalesTarget":"Sales (Target)"}).round(1),
    ...
    with t2:
        ...
        with mtab1:
            st.plotly_chart(_metric_area(f, "SalesActual", "Daily Sales (Area)"),
                            use_container_width=True, config={"displayModeBar": False})
        with mtab2:
            st.plotly_chart(_metric_area(f, "NOBActual", "Number of Baskets (Area)"),
                            use_container_width=True, config={"displayModeBar": False})
        with mtab3:
            st.plotly_chart(_metric_area(f, "ABVActual", "Average Basket Value (Area)"),
                            use_container_width=True, config={"displayModeBar": False})
